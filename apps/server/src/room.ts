...
export class GameRoom extends EventEmitter {
  ...
  private beginRound(dealerSeat: number): void {
    this.clearTimer(); const progress = this.tableProgress; if (!progress) throw new Error("庄家尚未确定"); const wall = createMahjongWall();
    ...
    this.round = { handNumber: currentHandNumber, dealerSeat, phase: "ROLLING_FOR_WALL", currentSeat: dealerSeat, wall, jokerIndicator: null, jokerType: null, wallRoll: null, wallBreak: null, lastAction: { type: "round-created", handNumber: currentHandNumber }, reactionWindow: null, settlement: null, version: (this.round?.version ?? 0) + 1, lastDrawWasReplacement: false, floorMultiplier: this.nextFloorMultiplier, dealerRuntime: null, outcome: null, liveWall: [], deadWall: [] };
    this.nextFloorMultiplier = 1; this.emit("event", { type: "ROUND_STARTED", data: { handNumber: currentHandNumber, dealerSeat, progress } }); this.emitState(); this.startWallRollTimeout();
  }
