...
export interface RoundRuntime {
  ...
  wallBreak: ReturnType<typeof breakMahjongWall> | null; lastAction: Record<string, unknown> | null; latestDiscard: { playerId: string; tileId: string } | null; reactionWindow: ReactionWindow | null;
...
export class GameRoom extends EventEmitter {
  ...
  private waitingRound(): RoundRuntime { return { handNumber: 0, dealerSeat: -1, phase: "WAITING_FOR_PLAYERS", currentSeat: 0, wall: null, jokerIndicator: null, jokerType: null, wallRoll: null, wallBreak: null, lastAction: null, latestDiscard: null, reactionWindow: null, settlement: null, version: 0, lastDrawWasReplacement: false, floorMultiplier: 1, dealerRuntime: null, outcome: null, liveWall: [], deadWall: [] }; }
  ...
  private beginRound(dealerSeat: number): void {
    ...
    this.round = { handNumber: currentHandNumber, dealerSeat, phase: "ROLLING_FOR_WALL", currentSeat: dealerSeat, wall, jokerIndicator: null, jokerType: null, wallRoll: null, wallBreak: null, lastAction: { type: "round-created", handNumber: currentHandNumber }, latestDiscard: null, reactionWindow: null, settlement: null, version: (this.round?.version ?? 0) + 1, lastDrawWasReplacement: false, floorMultiplier, dealerRuntime: null, outcome: null, liveWall: [], deadWall: [] };
  ...
  discard(playerId: string, tileId: string, actionId: string, version: number): void {
    const round = this.requireRound(); this.assertVersion(version); if (round.phase !== "WAITING_FOR_DISCARD") throw new Error("当前不是出牌阶段"); const player = this.getPlayer(playerId); if (player.seat !== round.currentSeat) throw new Error("还没轮到你"); const tileIndex = player.hand.findIndex((tile) => tile.tileId === tileId); if (tileIndex < 0) throw new Error("这张牌不在你的手里"); const tile = player.hand[tileIndex]!; if (isJoker(tile.type, round.jokerType)) throw new Error("默认规则不允许主动打出混儿"); player.hand.splice(tileIndex, 1); player.discards.push(tile); player.isAutopilot = false; round.latestDiscard = { playerId, tileId: tile.tileId }; round.lastAction = { type: "discard", playerId, tileId: tile.tileId, tileType: tile.type, actionId }; round.version += 1; this.clearTimer(); this.openReactionWindow(tile, player.seat, actionId);
  }
  ...
  private removeDiscardedTile(player: InternalPlayer, tile: Tile): void { const index = player.discards.findIndex((candidate) => candidate.tileId === tile.tileId); if (index >= 0) player.discards.splice(index, 1); const round = this.requireRound(); if (round.latestDiscard?.tileId === tile.tileId) round.latestDiscard = null; }
