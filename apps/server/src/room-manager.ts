...
export class RoomManager {
  ...
  disconnect(roomId: string, playerId: string, socketId?: string): void { const room = this.rooms.get(roomId); if (!room) return; room.disconnect(playerId, socketId); if (room.activeHumanCount() === 0 && !this.cleanupTimers.has(roomId)) { const timer = setTimeout(() => this.destroy(roomId), 5 * 60 * 1000); this.cleanupTimers.set(roomId, timer); } }
  leave(roomId: string, playerId: string): void { const room = this.rooms.get(roomId); if (!room) return; room.removePlayer(playerId); if (room.humanCount() === 0 || room.playerCount() === 0) this.destroy(roomId); else if (room.activeHumanCount() === 0 && !this.cleanupTimers.has(roomId)) { const timer = setTimeout(() => this.destroy(roomId), 5 * 60 * 1000); this.cleanupTimers.set(roomId, timer); } }
