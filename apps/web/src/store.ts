...
const readIdentity = (): Identity | null => { try { const raw = localStorage.getItem(IDENTITY_KEY); return raw ? JSON.parse(raw) as Identity : null; } catch { return null; } };
const saveIdentity = (identity: Identity): void => localStorage.setItem(IDENTITY_KEY, JSON.stringify(identity));
const clearIdentity = (): void => localStorage.removeItem(IDENTITY_KEY);
...
export const useGameStore = create<Store>((set, get) => {
  ...
  socket.on("room:state", (state) => set({ state, error: null }));
